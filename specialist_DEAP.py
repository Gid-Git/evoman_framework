...
# Create a Fitness and Individual class using DEAP
...
class EvolutionaryAlgorithm:
    ...
    def train(self):
        if not os.path.exists(self.experiment_name + '/evoman_solstate'):
            ...
            best = np.argmax(self.fit_pop)
            # finds the last generation number
        ...
        # Create a DEAP toolbox and define the necessary functions
        toolbox = base.Toolbox()
        toolbox.register("attr_float", np.random.uniform, self.dom_l, self.dom_u, self.n_vars)
        toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_float, self.npop)
        ...
        toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=self.mutation, indpb=0.2)
