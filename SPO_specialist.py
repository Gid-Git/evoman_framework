...
param_change = {
    ...
    'population_size': {'range': (10, 50), 'type': 'int'},
}
...
# Normalize ranges and generate initial design points. We want population_size only for even numbers (not odd numbers). For example, if we evaluate it between 10 and 100, we want it to test 10, 12 or 98 but not 99 or 13 numbers.
...
# Ensure population_size is even in initial design
for i in range(n_samples):
    param_index = list(param_change.keys()).index('population_size')
    scaled_design[i, param_index] = int(round(scaled_design[i, param_index]))
    scaled_design[i, param_index] += scaled_design[i, param_index] % 2  # Make it even
...
for iteration in range(n_iterations):
    ...
    if 'population_size' in varying_parameters:
        varying_parameters['population_size'] = int(round(varying_parameters['population_size']))
        varying_parameters['population_size'] += varying_parameters['population_size'] % 2  # Make it even
