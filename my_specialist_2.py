    ...
    def crossover(self):
        ...
        for p in range(0, self.pop.shape[0], 2):
            ...
            for f in range(n_offspring):
                # decide the random number crossover points
                random_n_points = np.random.randint(1, self.n_vars)

                # choose random crossover points
                crossover_points = np.sort(np.random.choice(self.n_vars, random_n_points, replace=False))

                start = 0
                for index, point in enumerate(crossover_points):
                    if index % 2 == 0:
                        offspring[f][start:point] = p1[start:point]
                    else:
                        offspring[f][start:point] = p2[start:point]
                    start = point

                if random_n_points % 2 == 0:
                    offspring[f][start:] = p1[start:]
                else:
                    offspring[f][start:] = p2[start:]

                # Non-uniform Gaussian mutation, applied gene-wise
                for i in range(self.n_vars):
                    if np.random.uniform(0, 1) <= self.mutation:
                        sigma = self.get_sigma()
                        offspring[f][i] += np.random.normal(0, sigma)

                    offspring[f][i] = self.limits(offspring[f][i])
